...
#=========================body=====
    ...
    def crawl_by_date_range(self, start_date: str, end_date: str):
        self.results = []
        ...
        total_expected = len(target_dates_set) * 5
        ...
        while current_d >= end_date_obj:
            ...
            tqdm.write(f"\nĐang quét trang: {current_url}")

            links = self._get_links_from_category_page(current_url, num_articles=5)

            if not links:
                tqdm.write(f"Không tìm thấy bài báo cho ngày {current_date_str}. Chuyển ngày tiếp.")
                current_d -= timedelta(days=1)
                continue
            ...
            for link in links:
                ...
                if result:
                    ...
                    with self.results_lock:
                        if crawled_dates_counter[date_str] < 5:
                            crawled_dates_counter[date_str] += 1
                            self.results.append(result)
                            pbar.update(1)
                            tqdm.write(f"✓ Tìm thấy [{date_str}] ({crawled_dates_counter[date_str]}/5): {result['title'][:60]}...")
